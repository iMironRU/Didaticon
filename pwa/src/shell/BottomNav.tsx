...
export function BottomNav({ tabs, activeId, onTap }: Props) {
  return (
    <nav className="flex pt-1.5 pb-2.5 bg-elevated border-t border-line shrink-0">
      {tabs.map(it => {
        ...
        return (
          <button
            ...
            onClick={() => onTap(it.id)}
          >
            <span className={`relative inline-flex ${colorCls}`}>
              {it.icon}
            ...
            <span className={`text-[0.62rem] font-medium ${colorCls}`}>
              {it.label}
            </span>
          </button>
        );
