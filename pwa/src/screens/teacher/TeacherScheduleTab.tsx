...
const s: Record<string, React.CSSProperties> = {
  ...
  viewBtn:   { flex: 1, background: "var(--c-card)", border: "none", borderRadius: 8, padding: "7px 0", fontSize: "0.82rem", color: "var(--c-text-muted)", cursor: "pointer" },
  viewBtnActive: { background: "var(--c-accent)", color: "#fff" },
  datePicker:{ display: "flex", gap: 4, padding: "10px 16px", overflowX: "auto" },
  dayBtn:    { display: "flex", flexDirection: "column", alignItems: "center", minWidth: 40, padding: "6px 8px", background: "var(--c-card)", border: "none", borderRadius: 10, cursor: "pointer", gap: 2 },
  dayBtnSel: { background: "var(--c-accent)" },
  dayBtnToday:{ outline: "1.5px solid var(--c-accent)" },
  dayName:   { fontSize: "0.6rem", color: "var(--c-text-muted)", textTransform: "uppercase" as const },
  dayNum:    { fontSize: "0.9rem", fontWeight: 600, color: "var(--c-text-primary)" },
  dot:       { width: 4, height: 4, borderRadius: "50%" },
  list:      { flex: 1, padding: "0 16px 16px", display: "flex", flexDirection: "column", gap: 8 },
  dateHeader:{ fontSize: "0.75rem", color: "var(--c-text-muted)", fontWeight: 600, marginTop: 10, marginBottom: 2, textTransform: "capitalize" as const },
  empty:     { fontSize: "0.78rem", color: "var(--c-text-dim)", padding: "4px 0" },
  emptyDay:  { textAlign: "center" as const, color: "var(--c-text-dim)", fontSize: "0.85rem", marginTop: 40 },
  card:      { background: "var(--c-card)", borderRadius: 12, padding: "12px 14px", display: "flex", gap: 12, alignItems: "flex-start", cursor: "pointer" },
  cardRefused:{ opacity: 0.5 },
  cardTime:  { fontSize: "0.75rem", color: "var(--c-text-muted)", minWidth: 72, paddingTop: 2 },
  cardMain:  { flex: 1 },
  cardTitle: { fontSize: "0.92rem", fontWeight: 600, color: "var(--c-text-primary)", marginBottom: 4 },
  cardMeta:  { display: "flex", gap: 4, alignItems: "center", marginBottom: 3 },
  kindBadge: { fontSize: "0.68rem", background: "var(--c-progress-track)", borderRadius: 4, padding: "1px 6px", color: "var(--c-accent)" },
  metaItem:  { fontSize: "0.75rem", color: "var(--c-text-muted)" },
  cardGroups:{ fontSize: "0.78rem", color: "var(--c-text-muted)" },
  ...
  dialog:    { background: "var(--c-card)", width: "100%", borderRadius: "16px 16px 0 0", padding: "20px 20px 32px", display: "flex", flexDirection: "column", gap: 10 },
  dialogTitle: { fontSize: "1rem", fontWeight: 600, color: "var(--c-text-primary)" },
  dialogSubtitle: { fontSize: "0.8rem", color: "var(--c-text-muted)", marginTop: -4 },
  reasonBtn: { background: "var(--c-card)", border: "0.5px solid var(--c-border)", borderRadius: 10, padding: "12px 14px", fontSize: "0.9rem", color: "var(--c-text-primary)", cursor: "pointer", textAlign: "left" as const },
  reasonBtnSel: { border: "1.5px solid var(--c-accent)", color: "var(--c-accent)" },
  dialogActions: { display: "flex", gap: 8, marginTop: 4 },
  cancelBtn: { flex: 1, background: "var(--c-card)", border: "none", borderRadius: 10, padding: "13px 0", fontSize: "0.9rem", color: "var(--c-text-muted)", cursor: "pointer" },
